...
public class ScannerConcept {
    ...
    public static void main(String[] args) {
         ...
         * 2. 读到有效字符后将后效字符后面输入的空白作为分隔符 while has next() 或者结束符 if has next()
         * 3. while has next()换行符不会终止程序，制作为分隔符，需要手动设置break条件
         */
//        if(scanner.hasNext()){
...
//        }
     ...
     * 2. hasNext(), hasNextLine() while循环中换行符只是分隔符，不能终止输入阻塞，需要手动设置break条件
     * 3. hasNext()不能读入空白，hasNextLine()以行读入可以读入空格，hasNextLine可以结合字符串分割String.split()使用
     * 4. scanner.useDelimiter(pattern)用在scanner创建之后可以按照指定的pattern分割输入,如果指定分隔符没有空格，火车等，则默认分隔符将不再起效
     */
}
